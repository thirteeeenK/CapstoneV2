...
class MapService
{
    ...
    protected function hotelMarker(HotelModel $hotel, float $userLat, float $userLng): array
    {
        ...
        return [
            ...
            'image' => ResolvesImages::resolveImg(collect($hotel->images ?? [])->first()),
            'url' => '/hotels/'.$hotel->id,
    ...
    protected function activityMarker(ActivityModel $activity, float $userLat, float $userLng): array
    {
        ...
        if ($lat === null || $lng === null) {
            return [
                ...
                'lat' => null,
                'lng' => null,
                'address' => $activity->specific_address,
                'rate' => $activity->rate,
                'category' => $activity->category,
                'rating' => $activity->reviewSummary?->average_rating ?? null,
                'review_count' => $activity->reviewSummary?->total_reviews ?? 0,
                'image' => ResolvesImages::resolveActivityImage(collect($activity->images ?? [])->first(), $activity->activity_name, $activity->category),
                'url' => null,
                'distance_km' => null,
                'distance_label' => null,
                'no_location' => true,
