                        ...
                        <span class="flex items-center gap-0.5 text-amber-400">
                            @for ($i = 1; $i <= 5; $i++)
                                <span class="material-symbols-outlined text-[16px]"
                                    style="font-variation-settings: 'FILL' {{ $i <= (int) round((float) $summary->average_rating) ? 1 : 0 }}">star</span>
                            @endfor
                        </span>
                    </div>
                    <div class="flex items-center gap-2">
                            ...
                            <div class="flex gap-0.5 text-amber-400">
                                @for ($i = 1; $i <= 5; $i++)
                                    <span class="material-symbols-outlined text-[18px]"
                                        style="font-variation-settings: 'FILL' {{ $i <= (int) $review->rating ? 1 : 0 }}">star</span>
                                @endfor
                            </div>
                            <span
                                class="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-bold border {{ $sentimentTint[$review->sentiment] ?? $sentimentTint['neutral'] }}">
                                <span
                                    class="material-symbols-outlined text-[12px]">{{ $sentimentIcon[$review->sentiment] ?? 'sentiment_neutral' }}</span>
