<div x-data="{ sidebarOpen: false }">
    {{-- Mobile Top Bar --}}
    <div class="md:hidden sticky top-0 z-30 bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between shadow-xs">
        <a href="{{ route('dashboard') }}" class="flex items-center gap-2">
            <span class="font-headline font-black text-lg text-slate-900 tracking-tight">Sunny<span class="text-sky-600">Trips</span></span>
            <span class="text-[10px] font-bold uppercase tracking-widest text-sky-600 bg-sky-50 px-2 py-0.5 rounded border border-sky-200">Traveler</span>
        </a>
        <button @click="sidebarOpen = !sidebarOpen" class="p-2 rounded-lg text-slate-600 hover:bg-slate-100 focus:outline-none">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'"
        ...
        <div class="px-6 mb-6 flex items-center justify-between">
            <div>
                ...
                <p class="font-label text-[10px] uppercase tracking-[0.18em] text-sky-600 font-bold mt-0.5 flex items-center gap-1">
        ...
        @php
            $navGroups = [
                ...
                'Explore Catalog' => [
                    ['route' => 'destination.*', 'icon' => 'hotel', 'label' => 'Hotels & Sanctuaries', 'href' => url('/#destinations')],
                    ['route' => 'rooms.*', 'icon' => 'king_bed', 'label' => 'Rooms & Stays', 'href' => url('/#destinations')],
                    ['route' => 'experiences.*', 'icon' => 'explore', 'label' => 'Activities & Tours', 'href' => url('/#experiences')],
                    ['route' => 'addons.*', 'icon' => 'extension', 'label' => 'Transfers & Add-ons', 'href' => url('/#experiences')],
                ],
                'Personalization' => [
                    ...
                    <div class="space-y-1">
                        @foreach ($items as $item)
                            ...
                            <a href="{{ $item['href'] }}"
                                ...
                                <span class="material-symbols-outlined text-[20px] shrink-0 {{ $isActive ? 'text-sky-600' : 'text-slate-400 group-hover:text-sky-600' }}">
                                    {{ $item['icon'] }}
                                </span>
        ...
        <div class="mt-auto px-4 pt-4 border-t border-slate-200 space-y-3">
            <div class="flex items-center gap-3 px-2">
                <div class="w-9 h-9 rounded-xl bg-sky-500/10 border border-sky-200 text-sky-700 font-bold flex items-center justify-center text-xs shrink-0">
