@php
    ...
    foreach ($cartItems as $cItem) {
        ...
        } elseif ($cItem->item_type === 'addon' && $cItem->itemable) {
            $maxPax = method_exists($cItem->itemable, 'getMaxPax') ? $cItem->itemable->getMaxPax() : 10;
            $itemManifests[] = [
                'cart_item_id' => $cItem->id,
                'type' => 'addon',
                ...
                'base_pax' => 0,
                'max_pax' => $maxPax,
                'field_name' => 'addon_manifest_' . $cItem->id,
            ];
        } elseif ($cItem->item_type === 'activity' && $cItem->itemable) {
            $maxPax = method_exists($cItem->itemable, 'getMaxCapacityInt') ? $cItem->itemable->getMaxCapacityInt() : 20;
            $itemManifests[] = [
                'cart_item_id' => $cItem->id,
                'type' => 'activity',
                'label' => $cItem->item_title,
                ...
                'base_pax' => 0,
                'max_pax' => $maxPax,
                'field_name' => 'activity_manifest_' . $cItem->id,
            ];
        }
    }
                ...
                @foreach($itemManifests as $manifest)
                    <div x-data="itemManifestBuilder({
                        ...
                        <div class="flex items-center gap-3 border-b border-slate-100 pb-4">
                            <div class="w-10 h-10 rounded-2xl {{ $manifest['type'] === 'addon' ? 'bg-violet-50 text-violet-600 border-violet-200' : ($manifest['type'] === 'room' ? 'bg-amber-50 text-amber-600 border-amber-200' : 'bg-teal-50 text-teal-600 border-teal-200') }} border flex items-center justify-center font-bold">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-500">
                                    <span class="font-semibold {{ $manifest['type'] === 'addon' ? 'text-violet-700' : ($manifest['type'] === 'room' ? 'text-amber-800' : 'text-teal-700') }}">{{ $manifest['label'] }}</span>
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            <template x-for="(p, i) in passengers" :key="i">
                                <div class="bg-slate-50/80 p-4 sm:p-5 rounded-2xl border border-slate-200/70 space-y-3 transition duration-200 hover:border-slate-300">
                                    <div class="flex items-center justify-between">
                                        <span class="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                                            <span class="material-symbols-outlined text-[16px] text-sky-600">person</span>
                                            <span x-text="i === 0 ? 'Lead Traveler' : 'Accompanying Guest ' + (i + 1)"></span>
                                        </span>
                                        <button type="button" x-show="i > 0"
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <input type="text" x-model="p.full_name"
                                                   @input="notifyManifestChange()"
                        ...
                        <div class="pt-2 flex items-center justify-between">
                            <button type="button" @click="addPassenger()"
                                    :disabled="passengers.length >= maxPax"
                                ...
                                <span x-text="passengers.length >= maxPax ? 'Max Occupancy Limit Reached (' + maxPax + ' Guests)' : '+ Add Accompanying Guest'"></span>
                            </button>
                            <span class="text-[11px] font-semibold text-slate-500">
                                <template x-if="basePax > 0">
                                    <span>Base capacity: <strong class="text-slate-800" x-text="basePax + ' Pax'"></strong> · </span>
                                </template>
