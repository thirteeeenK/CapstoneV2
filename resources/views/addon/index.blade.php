<x-frontend.layout title="Transfers & Travel Add-ons — SunnyTrips">
    ...
    <div x-data="{ activeDestId: '{{ request('destination_id') ?: 'all' }}', previewAddon: null }"
                    ...
                    @foreach($addons as $addon)
                        ...
                        <div x-show="activeDestId === 'all' || String(activeDestId) === '{{ $addon->destination_id }}'"
                            ...
                            <div class="pt-6 border-t border-slate-100 mt-4 grid grid-cols-2 gap-2">
                                ...
                                <button type="button" @click="window.addToCart('addon', {{ $addon->id }})"
        ...
        <div x-show="previewAddon" x-transition.opacity @keydown.escape.window="previewAddon = null"
            ...
            <div @click.away="previewAddon = null"
                                        ...
                                        <template x-for="(tier, idx) in previewAddon.pricing_tiers" :key="idx">
                                            <tr class="hover:bg-slate-50/50">
                                                ...
                                                <td class="px-4 py-2 text-right font-bold text-emerald-600 font-mono">
                                                    ₱<span
                                                        x-text="Number(tier.rate).toLocaleString('en-US', {minimumFractionDigits: 2})"></span>
                                                </td>
                                            </tr>
                                        </template>
                                    </tbody>
                                ...
                                <template x-for="(surcharge, idx) in previewAddon.surcharges" :key="idx">
                                    <div class="flex items-center justify-between text-xs font-medium">
                                        ...
                                        <span class="font-bold font-mono">+₱<span
                                                x-text="surcharge.amount"></span></span>
                                    </div>
                                </template>
                            </div>
                        </div>
                ...
                <div
                    ...
                    <button type="button" @click="window.addToCart('addon', previewAddon.id); previewAddon = null;"
