    ...
    <div class="pb-12" x-data="{
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-1">Manage user accounts, monitor AI chatbot abuse flags, and handle
        ...
        <div class="mb-6 bg-white p-4 border border-slate-200 rounded-lg shadow-sm space-y-4">
            <div class="flex flex-col gap-4">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <a href="{{ route('admin.users.index', array_filter(['tab' => 'all', 'search' => $search])) }}"
                        ...
                        <span :class="tab === 'all' ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-700'"
                            class="px-1.5 py-0.2 rounded-full text-[10px]">{{ $allCount }}</span>
                    </a>

                    <select @change="setTab($event.target.value || 'all')"
                        :value="['flagged', 'warned', 'temporary', 'permanent'].includes(tab) ? tab : ''"
                        class="px-3 py-1.5 rounded-md text-xs font-medium bg-slate-100 text-slate-600 border border-transparent hover:bg-slate-200 focus:outline-none focus:ring-2 focus:ring-ocean-500/20 focus:border-ocean-600 transition-colors cursor-pointer">
                ...
                <form action="{{ route('admin.users.index') }}" method="GET" @submit.prevent="fetchResults()"
                    class="flex items-center gap-2 w-full lg:w-auto lg:self-end">
