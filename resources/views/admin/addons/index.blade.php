...
@section('title', 'Add-ons Management | SunnyTrips Admin')
...
@section('content')
    <div class="pb-12"
         x-data="{
             ...
             loading: false,
             timer: null,
             baseUrl: @js(route('admin.addons.index')),
             async fetchResults() {
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-xl font-bold text-slate-900">Transfers & Add-ons</h1>
                <p class="text-xs text-slate-500 mt-1">Manage destination Add-ons and Transfer Related</p>
            </div>
            <a href="{{ route('admin.addons.create') }}" class="inline-flex items-center gap-1.5 h-9 px-3 rounded-md bg-ocean-600 hover:bg-ocean-700 text-white text-xs font-semibold shadow-sm transition-colors self-start sm:self-auto">
        ...
        <div class="mb-6 bg-white p-4 border border-slate-200 rounded-lg shadow-sm space-y-4">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2 overflow-x-auto pb-1 lg:pb-0">
                    ...
                    <a href="{{ route('admin.addons.index', array_filter(['search' => $search])) }}"
                    ...
                    @foreach($destinations as $dest)
                        <a href="{{ route('admin.addons.index', array_filter(['destination_id' => $dest->id, 'search' => $search])) }}"
                ...
                <!-- Search Form -->
                <form action="{{ route('admin.addons.index') }}" method="GET" @submit.prevent="fetchResults()" class="flex items-center gap-2 w-full lg:w-auto">
                    ...
                    <div class="relative w-full sm:w-64">
                        ...
                        <input type="text" name="search" x-model="search" value="{{ $search }}"
                               placeholder="Search Add-ons and Transfers..."
                        ...
                        <a href="{{ route('admin.addons.index', array_filter(['destination_id' => $selectedDestinationId])) }}"
        ...
        <div>
            <div :class="loading ? 'opacity-60 transition-opacity' : 'transition-opacity'">
                @include('admin.addons._table', compact('addons', 'selectedDestinationId'))
            </div>
        </div>
    </div>
