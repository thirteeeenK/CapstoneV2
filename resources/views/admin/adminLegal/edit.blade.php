...
@section('title', "Edit $document->title | SunnyTrips Admin")
    ...
    <div class="pb-12 max-w-4xl mx-auto">
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <h1 class="text-xl font-bold text-slate-900">Edit: {{ $document->title }}</h1>
                <p class="text-xs text-slate-500 mt-1">Update the title, version, and individual sections shown on the public page.</p>
            </div>
        </div>
        ...
        <div class="bg-white border border-slate-200 rounded-lg shadow-sm p-6">
            <form action="{{ route('admin.legal.update', $document->key) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" id="version" name="version" value="{{ old('version', $document->version) }}" required
                               placeholder="e.g. 1.1"
                               class="w-full bg-slate-50 border border-slate-300 rounded-md px-3 py-2 text-xs text-slate-900 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all" />
                        <p class="text-[10px] text-slate-400">Bump this when the content changes materially. Users may be asked to re-consent to the new version.</p>
                    </div>
                </div>
                ...
                <div
                    x-data="{
                        sections: {{ Js::from(old('sections', $document->content ?? [])) }},
                        addSection() {
