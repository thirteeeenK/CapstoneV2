<x-frontend.layout title="I'm Feeling Lucky | SunnyTrips">
    <div x-data="luckyManager()" class="min-h-screen bg-slate-50 text-slate-900 font-body py-10 sm:py-16 relative overflow-hidden">
        
        {{-- Background Soft Ambient Mesh Glows --}}
        <div class="absolute top-12 left-1/4 w-[500px] h-[300px] bg-ocean-200/40 blur-3xl rounded-full pointer-events-none"></div>
        <div class="absolute bottom-20 right-1/4 w-[450px] h-[350px] bg-sky-200/30 blur-3xl rounded-full pointer-events-none"></div>
        ...
        <div id="lucky-root" class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10 relative z-10">
            ...
            <div class="text-center space-y-3 max-w-2xl mx-auto">
                ...
                <p class="text-slate-500 text-xs sm:text-sm font-body leading-relaxed">
                    Generate an instant, curated trip package — hotel stay and experiences matched to your budget and travel preferences.
                </p>
            </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-6 sm:p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
                    ...
                    {{-- Destination --}}
                    <div>
                        <label for="lucky-destination" class="block font-bold text-slate-700 mb-1.5 flex items-center gap-1">
                    ...
                    {{-- Hotel Category --}}
                    <div>
                        <label for="lucky-category" class="block font-bold text-slate-700 mb-1.5 flex items-center gap-1">
                    ...
                    {{-- Activity Count --}}
                    <div>
                        <label for="lucky-activities" class="block font-bold text-slate-700 mb-1.5 flex items-center gap-1">
                            <span class="material-symbols-outlined text-[15px] text-ocean-600">format_list_bulleted</span>
            ...
            <div class="text-center">
                <button type="button" @click="shuffle()" :disabled="loading || accepting"
                    ...
                    <span x-text="loading ? 'Generating surprise package…' : (itinerary ? 'Shuffle New Itinerary' : 'Generate Surprise Itinerary')"></span>
                </button>

                <template x-if="error">
                    <div class="mt-4 text-xs font-semibold text-rose-700 bg-rose-50 border border-rose-200 rounded-xl px-4 py-3 max-w-md mx-auto flex items-center gap-2">
                ...
                <div class="bg-white rounded-3xl border border-slate-200/80 shadow-md overflow-hidden animate-fade-up">
                    ...
                    <div class="relative bg-slate-900 text-white p-6 sm:p-8 overflow-hidden">
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-4 relative z-10">
                            <div class="space-y-1">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-ocean-500/20 text-ocean-300 text-[10px] font-bold uppercase tracking-wider border border-ocean-400/30">
                                    <span class="material-symbols-outlined text-[14px]">explore</span>
                                    <span>Surprise Package Generated</span>
                                </span>
                                <h2 class="font-headline text-2xl sm:text-3xl font-black text-white" x-text="itinerary.destination.name"></h2>
                                <p class="text-xs text-slate-300 font-medium">
                                    <span x-text="itinerary.nights + ' night' + (itinerary.nights > 1 ? 's' : '')"></span>
                                    <span x-text="' • ' + itinerary.pax + ' guest' + (itinerary.pax > 1 ? 's' : '')"></span>
                            ...
                            <div class="text-left sm:text-right space-y-1">
                                <template x-if="itinerary.budget_exceeded">
                                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-400/30 text-[10px] font-bold uppercase tracking-wider">
                                        <span class="material-symbols-outlined text-[13px]">warning</span>
                                        Over Budget
                                    </span>
                                </template>
                                <template x-if="!itinerary.budget_exceeded">
                                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-400/30 text-[10px] font-bold uppercase tracking-wider">
                                        <span class="material-symbols-outlined text-[13px]">check_circle</span>
                                        Within Budget
                                    </span>
                                </template>
                                <p class="font-headline text-3xl font-black text-emerald-400" x-text="itinerary.formatted_total"></p>
                                <p class="text-[11px] text-slate-400 font-medium" x-text="'Target budget: ' + itinerary.formatted_budget"></p>
                            </div>
                        </div>

                        <template x-if="itinerary.budget_notice">
                            <p class="mt-4 text-xs text-amber-200 bg-amber-950/40 border border-amber-400/30 rounded-xl p-3 flex items-center gap-2" x-text="itinerary.budget_notice"></p>
                        </template>
                    </div>
                    ...
                    <div class="p-6 sm:p-8 space-y-8">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex flex-col sm:flex-row gap-4 bg-slate-50 rounded-2xl p-4 border border-slate-200/80 items-center sm:items-start">
                                ...
                                <div class="flex-1 min-w-0 space-y-1.5">
                                    <p class="text-xs font-bold text-ocean-600 uppercase tracking-wider" x-text="itinerary.hotel.hotel_name"></p>
                                    <h4 class="text-base font-bold text-slate-900 font-headline" x-text="itinerary.room.room_name"></h4>
                                    
                                    <div class="flex flex-wrap gap-1.5 pt-1">
                                        <template x-if="itinerary.hotel.type">
                                            <span class="inline-flex items-center gap-1 rounded-md bg-white border border-slate-200 px-2 py-0.5 text-[10px] font-medium text-slate-700 capitalize" x-text="itinerary.hotel.type"></span>
                                        </template>
                                        <template x-if="itinerary.room.bed_configuration">
                                            <span class="inline-flex items-center gap-1 rounded-md bg-white border border-slate-200 px-2 py-0.5 text-[10px] font-medium text-slate-700">
                                                <span class="material-symbols-outlined text-[12px] text-slate-400">king_bed</span>
                                ...
                                <div class="text-left sm:text-right shrink-0 border-t sm:border-t-0 border-slate-200 pt-2 sm:pt-0 w-full sm:w-auto">
                                    <span class="block text-[11px] text-slate-400 font-medium" x-text="itinerary.room.formatted_nightly_rate + ' / night'"></span>
                                    <span class="block text-lg font-black text-slate-900 font-headline mt-0.5" x-text="'₱' + (Number(itinerary.room.nightly_rate) * Number(itinerary.nights)).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })"></span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center justify-between border-b border-slate-100 pb-2.5">
                                ...
                                <span class="text-xs font-bold text-ocean-600 bg-ocean-50 px-2.5 py-0.5 rounded-full border border-ocean-100"
                            ...
                            <div class="grid grid-cols-1 gap-3">
                                <template x-for="(activity, i) in itinerary.activities" :key="activity.id">
                                    <div class="flex items-center gap-3.5 bg-slate-50 rounded-2xl p-4 border border-slate-200/80 hover:bg-slate-100/60 transition-colors">
                                        <span class="w-7 h-7 rounded-xl bg-ocean-600 text-white text-xs font-bold flex items-center justify-center shrink-0 font-headline" x-text="i + 1"></span>
                                        <div class="flex-1 min-w-0">
                                            <p class="text-xs sm:text-sm font-bold text-slate-900 font-headline" x-text="activity.activity_name"></p>
                                            <div class="flex flex-wrap gap-1.5 mt-1">
                                                <span class="inline-flex items-center rounded-md bg-white border border-slate-200 px-2 py-0.5 text-[10px] font-medium text-slate-700 capitalize" x-text="activity.category"></span>
                                                <span class="inline-flex items-center rounded-md bg-white border border-slate-200 px-2 py-0.5 text-[10px] font-medium text-slate-700 capitalize" x-text="activity.activity_level"></span>
                                                <template x-if="activity.duration">
                                                    <span class="inline-flex items-center rounded-md bg-white border border-slate-200 px-2 py-0.5 text-[10px] font-medium text-slate-700" x-text="activity.duration"></span>
                                                </template>
                                            </div>
                                        </div>
                                        <div class="text-right shrink-0">
                                            <span class="block text-[10px] text-slate-400 font-medium">for <span x-text="itinerary.pax"></span> pax</span>
                                            <span class="block text-xs sm:text-sm font-bold text-slate-900 font-headline mt-0.5" x-text="activity.formatted_rate"></span>
                                        </div>
                                    </div>
                                </template>
                            </div>
                        ...
                        <div class="rounded-2xl bg-slate-900 text-white p-6 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-lg">
                            <div class="text-center sm:text-left">
                                <p class="text-xs text-slate-400 font-medium">Estimated Itinerary Total</p>
                                <p class="font-headline text-3xl font-black text-emerald-400 mt-0.5" x-text="itinerary.formatted_total"></p>
                            </div>
