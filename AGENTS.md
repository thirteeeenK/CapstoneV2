...
- Embeddings are stored in real pgvector `vector(3072)` columns for rooms, hotels, activities, add_ons, users (preferences), and packages (migrated from text). Parse/serialize via `GeminiService` helpers. `RecommendationController` ranks hotels/activities with `GeminiService::rankRecommendations` and shows a 2-3 sentence section overview ("why these were picked") under the top-stays and top-experiences headings via `RecommendationExplainer`. Overviews are persisted per destination in `user_preferences.recommendation_explanations` (jsonb) and only regenerate after `OnboardingController::store()` calls `invalidateForUser()` — the first dashboard view after a preference update does one batched Gemini call per destination, then it's a DB read; a deterministic tag-match fallback covers Gemini outages; a "Personalize now" CTA shows when no `user_preferences` row exists.
- Onboarding quiz answers are persisted in a structured `user_preferences` row (vibes, amenities, destination, traveler_type, notes) alongside the embedding in `users.preferences_embedding`. The onboarding form options (vibes, traveler types, amenities) are admin-managed via `OnboardingOption` and rendered from the database; see `AdminOnboardingOptionController` and `routes/adminOnboardingOptionRoute.php`.
